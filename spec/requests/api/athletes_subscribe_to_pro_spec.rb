...
RSpec.describe Api::AthletesController, type: :request do
  ...
  describe 'POST subscribe_to_pro' do
    it 'should return 404 when the requested subscription plan does not exist' do
      post '/api/athletes/123/subscribe_to_pro', params: { subscriptionPlanId: '11111-aaaa-bbbb-2222-1111111111' }
      expect(response).to have_http_status(404)
    end

    it 'should return 404 when the requested athlete does not exist' do
      post '/api/athletes/12345678/subscribe_to_pro'
      expect(response).to have_http_status(404)
    end

    it 'should return 403 when requested athlete is not the current user' do
      setup_cookie(nil)
      post '/api/athletes/9123806/subscribe_to_pro', params: { subscriptionPlanId: '43ab44e0-49c2-46cc-9cfd-66591f9f708c' }
